...
import { Filter, ChevronDown, FileText, ShieldCheck, Globe, SearchX, RotateCcw, Plus, Upload, Pencil, Trash2, MoreVertical } from "lucide-react"
...
export default function ReportsPage() {
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      try {
        addReport(healthcheckToReportData(JSON.parse(String(reader.result)) as HealthcheckJSON))
        setMenuOpen(null)
        router.push("/builder")
      } catch {
              ...
              {isAdmin && (
                <>
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2.5 rounded-lg border border-border bg-card text-sm font-medium hover:bg-secondary active-scale-sm"
                  >
